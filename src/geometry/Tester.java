		...
		for(int i=0; i<nodesList.size(); i++){
			for(int j=0; j<nodesList.size(); j++){
				if(nodesList.get(i).isConnected(nodesList.get(j))){
					System.out.println("Node "+(i+1)+": "+nodesList.get(i));
					System.out.println("Node "+(j+1)+": "+nodesList.get(j));
					System.out.println("edge: "+nodesList.get(i).getConnectionEdge(nodesList.get(j)));
					System.out.println();
				}
			}
		}
		System.out.println("center's connections: "+net.getCenter().getEdgesList());

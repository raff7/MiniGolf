...
public class CollisionHandler {
	...
	public void checkWallCollision(){
		...
		for(int n = 0; n<obstacles.size(); n++){
			...
			//Looking for the orthogonal projection of the center onto the wall (Technique found online)
			...
			double normalDistance = Math.sqrt( Math.pow((orthoProj[0]-ball.getX()), 2) + Math.pow(orthoProj[1]-ball.getY(), 2) + Math.pow(orthoProj[2]-ball.getZ(), 2) );
			double movementLength = Math.sqrt(Math.pow(ball.movement[0], 2) + Math.pow(ball.movement[1], 2) + Math.pow(ball.movement[2], 2));
			
				System.out.println("normalDistance "+normalDistance);
				//System.out.println("coordinates "+ball.getX()+" "+ball.getY()+" "+" "+ball.getZ());
				//System.out.println("movement length "+movementLength);
			//System.out.println("normal distance "+normalDistance );
			if(Math.abs(normalDistance) < 20){
				...
				if(dotProduct > 0 && untilProjectLength < lineVectorLength){
					...
					//ball.setMovement(new double[]{0,0,0});
					...
					/*ball.setMovement(new double[]{0,0,0});
					if(!ball.collision)
						ball.setMovement(ball.newMovement);
					ball.collision=true;*/
				}
			}
		}
	...
	public double[] getNewVectorAfterCollision(Point point1, Point point2, double[] collisionPoint){
		...
		double[] incomingPoint = new double[]{collisionPoint[0]-ball.movement[0],collisionPoint[1]-ball.movement[1],collisionPoint[2]-ball.movement[2]};
		...
		//Ortho projection of the incoming point onto the wall
		double[] orthoProj = new double[]{x,y,z};;
		...
		if(true){
			...
			//System.out.println("contactPoint "+ collisionPoint[0]+" "+collisionPoint[1]+" "+collisionPoint[2]);
			//System.out.println("outcomingPoint: "+outcomingPoint[0]+" "+outcomingPoint[1]+" "+outcomingPoint[2]);
			System.out.println("old movement "+ball.movement[0] + " "+ball.movement[1] + " " + ball.movement[2]);
